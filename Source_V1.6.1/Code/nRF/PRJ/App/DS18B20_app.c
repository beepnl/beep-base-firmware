...
static void DS18B20_App_convert(CONTROL_SOURCE source, MEASUREMENT_RESULT_s * conv)
{
	...
	for(i=0; i<app.device_count; i++)
	{
		conv->result.ds18B20.temperatures[i] = (int16_t)(app.dev[i].temp * 100.0);
        #if 0
            // NRF_LOG_INFO("Lora DS18B20[%u] temp: "NRF_LOG_FLOAT_MARKER"C, converted: %i", 
                                i,
                                NRF_LOG_FLOAT(conv->result.ds18B20.temperatures[i]),
                                conv->result.ds18B20.temperatures[i]);
        #endif
	}
}
...
static void DS18B20_clearErrorCount(uint8_t index)
{
...
void DS18B20_App_NewState(DS18B20_STATES_s newState)
{
	app.state = newState;
		...
		NRF_LOG_FLUSH();
	#endif
    DS18B20_clearErrorCount(UINT8_MAX);
...
void DS18B20_clearIndex(uint8_t index)
{
	if(index >= DS18B20_SENSORS_MAX)
	{
		...
		for(i=0; i < DS18B20_SENSORS_MAX; i++)
		{
			memset((sDS18B20*)&app.dev[i], 0, sizeof(sDS18B20));
		}
	}
	else
	{
		// Clear a specific index
		memset((sDS18B20*)&app.dev[index], 0, sizeof(sDS18B20));
	}
}
...
/*
* Returns false when in the STOP, IDLE or UNKNOWN state and shuts down power to the DS18B20(s). Returns true when in other states
*/
...
bool DS18B20_App_busy(void)
{
	...
	bool DS18B20_App_IsBusy = (app.state == DS_STOP || app.state == DS_IDLE || app.state == DS_UNKNOWN_STATE) ? false : true;
	
	if(!DS18B20_App_IsBusy)
	powerApp_Enable(false, PWR_DS18B20);
	
	return DS18B20_App_IsBusy;
...
static uint32_t DS18B20_App_index_conversion(void)
{
	...
	{
		ROM                         = (uint8_t *)app.dev[app.convIndex].ROM;
	...
	switch(app.state)
	{
		...
		case DS_SEARCH:
		{
			...
			if(search_result)
			{
				...
				memcpy((sDS18B20*) app.dev[app.device_count].ROM, ROM, DS18B20_ROM_LENGHT);
			...
			{
                if(app.device_count == 0 && app.scanAttempts < DS18B20_ERROR_COUNT_MAX)
                {
                    // Reload this state to reset the start timestamp
                    DS18B20_App_NewState(DS_SEARCH);
                    // NRF_LOG_INFO("DS18B20 scan attempt %u failed", app.scanAttempts);
                }


				DS18B20_App_NewState((app.device_count == 0) ? DS_STOP : DS_READ_CONFIG);
				app.read_index = 0;
                #if 1 //DS18B20_LOG_ENABLE
		...
		case DS_READ_CONFIG:
		{
            ...
            dev = (sDS18B20*) &app.dev[app.read_index];
        ...
        case DS_START_TEMP_CONV:
		{
			...
			{
                ret = DS18B20_App_index_conversion();

                // Check if the temperature conversion could be started or no device was detected.
                if(ret != DS_SUCCESFULL)
                {
                    // Return to Idle when not in a valid operation mode.
                    DS18B20_App_NewState(DS_IDLE);
                    powerApp_Enable(false, PWR_DS18B20);
		...
		case DS_GET_TEMP_CONV_RESULT:
		{
			...
			if(dev->enabled && dev->errorCount < DS18B20_ERROR_COUNT_MAX)
			{
				...
				if(status == DS_SUCCESFULL)
				{
                    ...
                    #if 0
                        // NRF_LOG_INFO("Scratchpad[%u]= 0x%02X, 0x%02X", app.read_index, scratchPad[0], scratchPad[1]);
                    #endif
				...
				else
				{
					// Retry
					DS18B20_incErrorCount(app.read_index, status);
                    if(dev->errorCount >= DS18B20_ERROR_COUNT_MAX)
                    {
                        dev->temp = 100.0;
                    }
					return;
