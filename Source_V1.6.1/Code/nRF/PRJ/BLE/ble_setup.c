...
void gatt_evt_handler(nrf_ble_gatt_t * p_gatt, nrf_ble_gatt_evt_t const * p_evt)
{
    switch(p_evt->evt_id)
    {
        ...
        {
            m_beep.mtuDatalenght = p_evt->params.att_mtu_effective;
            m_beep.dataLenghtMax = p_evt->params.att_mtu_effective - OPCODE_LENGTH - HANDLE_LENGTH - 2;
            // NRF_LOG_INFO("effMTU=%u, MTUData=%u, maxDataLen=0x%0X/%u", p_evt->params.att_mtu_effective, p_evt->params.data_length, m_beep.dataLenghtMax, m_beep.dataLenghtMax);
            break;
        }
...
static void Beep_service_init(void * beep_callback_p)
{
    ...
    extern uint32_t flashApp_cmd_handler(app_flash_evt_type_t type, BEEP_protocol_s * prot);
...
uint32_t ble_TXdata_send(uint8_t * p_data, uint16_t length)
{
    return ble_beep_TXdata_send(&m_beep, p_data, length);
}

bool ble_beep_notificationEnabled(void)
{
    return beep_read_txNotificationState(&m_beep);
}

void ble_beep_clearPackerCounter(void)
{
    m_beep.packetCounter = 0;
}

uint16_t ble_beep_getPacketCounter(void)
{
    return m_beep.packetCounter;
}