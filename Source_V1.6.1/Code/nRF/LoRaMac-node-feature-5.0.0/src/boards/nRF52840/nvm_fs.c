...
fds_stat_t stat;
static uint32_t saveTimeCounter = 0;
...
static flash_struct_s flash_cfg =
{
	...
	.ds18B20Interval		= 10,
	.lorawanRatio			= 0,
	.mainSampleInterval_min	= DEFAULT_LORAWAN_MSG_INTERVAL_MINUTES,
    ...
    // Added Firmware version 1.4.0, revamped for 1.5.0
    .on_state           = ONOFF_STATE_DISABLED,
...
{
    // Reset the save counter for the time and date when the flash is written
    saveTimeCounter = 0;
    nvm.dataChanged = true;
...
static void nvm_fds_check_version(void)
{
    ...
    // For firmware updates below 1.5.0
    if(stored_fw_version < FIRMWARE_TO_UINT32_T(1, 5, 0)) // fw < 1.5.0
    {
        /* Check if the SQ_min 200 is enabled or not for fw versions 1.4.0, 1.4.1 and 1.4.2. When sq_state is low enable the reedswitch and the tilt 
         * sensor otherwise only the reedswitch needs to be enabled
         */
        if(stored_fw_version >= FIRMWARE_TO_UINT32_T(1, 4, 0) && !flash_cfg.on_state)
        {
            flash_cfg.onoff_control = ONOFF_CONTROL_MASK;
        }
        // Enable the tilt and reed switch for firmware versions below 1.4.0
        else if(stored_fw_version < FIRMWARE_TO_UINT32_T(1, 4, 0))
        {
            flash_cfg.onoff_control = ONOFF_CONTROL_MASK;
        }
        else
        {
            flash_cfg.onoff_control = ONOFF_CONTROL_DEFAULT_VALUE;
        }
        flash_cfg.on_state          = ONOFF_STATE_ENABLED;
        flash_cfg.lastTime          = TIME_DEFAULT;
    }
    else
    {
        if(stored_fw_version < FIRMWARE_TO_UINT32_T(1, 5, 1)) // 1.5.1 Enable the reedswitch by default
        {
            flash_cfg.onoff_control             = ONOFF_CONTROL_DEFAULT_VALUE;
        }
    }
...
static void nvm_fds_check_parameters(void)
{
    ...
    if(flash_cfg.lastTime < 0)
    {
        flash_cfg.lastTime = 0;
    }

    // When the on off control is either 0 or other bits than the mask are set, set it to default values.
    if(flash_cfg.onoff_control == 0 || flash_cfg.onoff_control & ~ONOFF_CONTROL_MASK)
    {
        flash_cfg.onoff_control = ONOFF_CONTROL_DEFAULT_VALUE;
    }
}
...
uint32_t nvm_fds_eeprom_get(BEEP_protocol_s * prot)
{
	...
	switch(prot->command)
	{
        ...
        case WRITE_ON_STATE:
        {
            prot->param.status.statusflag = flash_cfg.on_state;
            break;
        }
        ...
        {
            prot->param.status.statusflag = (flash_cfg.onoff_control & ONOFF_CONTROL_REEDSWITCH_MASK) ? 0x01 : 0x00;
            break;
        }
        ...
        {
            prot->param.status.statusflag = (flash_cfg.onoff_control & ONOFF_CONTROL_TILTSWITCH_MASK) ? 0x01 : 0x00;
            break;
        }
...
uint32_t nvm_fds_eeprom_set(BEEP_protocol_s * prot)
{
	...
	switch(prot->command)
	{
        ...
        {
            // fw 1.5.0 changed
            flash_cfg.on_state = prot->param.status.statusflag ? ONOFF_STATE_ENABLED : ONOFF_STATE_DISABLED;
            break;
        }
        ...
        case WRITE_SQ_MIN_STATE:
        {
            if(prot->param.status.statusflag)
            {
                flash_cfg.onoff_control |= ONOFF_CONTROL_TILTSWITCH_MASK;
            }
            else
            {
                flash_cfg.onoff_control &= ~ONOFF_CONTROL_TILTSWITCH_MASK;
        ...
        case WRITE_REED_STATE:
        {
            if(prot->param.status.statusflag)
            {
                flash_cfg.onoff_control |= ONOFF_CONTROL_REEDSWITCH_MASK;
            }
            else
            {
                flash_cfg.onoff_control &= ~ONOFF_CONTROL_REEDSWITCH_MASK;
...
{
    flash_cfg.on_state = (flash_cfg.on_state == ONOFF_STATE_DISABLED) ? ONOFF_STATE_ENABLED : ONOFF_STATE_DISABLED;
    NRF_LOG_INFO("Reedswitch new state = %s", flash_cfg.on_state ? "On" : "Off");
    nvm_fds_changed();
    return (bool) flash_cfg.on_state;
}
...
void nvm_setLastTime(const time_t newTime)
{
    ...
    // Force a flash save of the time and date every 48 hours since the last flash write.
    if(saveTimeCounter++ >= TIME_SAVE_INTERVAL_MIN)
    {
        saveTimeCounter = 0;
        nvm_fds_changed();
    }
...
{
    return (flash_cfg.onoff_control & ONOFF_CONTROL_REEDSWITCH_MASK) ? true : false;
}
...
/**
*@Brief:    This function reads the first three UICR registers in order to retrieve the hardware ID, otherwise provided by the ATECC
*/
void uicr_getHWid(void)
{
	BEEP_protocol_s prot;

    // Read the UICR registers
    uint32_t reg0 = NRF_UICR->CUSTOMER[UICR_ATECC_ID_1];
    uint32_t reg1 = NRF_UICR->CUSTOMER[UICR_ATECC_ID_2];
    uint32_t reg2 = NRF_UICR->CUSTOMER[UICR_ATECC_ID_3];

    prot.param.atecc_id.ROM[0] = (uint8_t) (reg0 >> 24); // Get the MSB
    prot.param.atecc_id.ROM[1] = (uint8_t) (reg0 >> 16); // Get 3th byte
    prot.param.atecc_id.ROM[2] = (uint8_t) (reg0 >>  8); // Get the 2nd byte
    prot.param.atecc_id.ROM[3] = (uint8_t) (reg0 >>  0); // Get the LSB
    prot.param.atecc_id.ROM[4] = (uint8_t) (reg1 >> 24); // Get the MSB
    prot.param.atecc_id.ROM[5] = (uint8_t) (reg1 >> 16); // Get 3th byte
    prot.param.atecc_id.ROM[6] = (uint8_t) (reg1 >>  8); // Get the 2nd byte
    prot.param.atecc_id.ROM[7] = (uint8_t) (reg1 >>  0); // Get the LSB
    prot.param.atecc_id.ROM[8] = (uint8_t) (reg2 >> 24); // Get the MSB
