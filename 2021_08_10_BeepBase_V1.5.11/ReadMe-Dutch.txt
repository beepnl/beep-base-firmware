2021-08-10 Release van versie 1.5.11
	-	Hardware ID kan nu in het UICR worden geschreven als de ATECC niet wordt bestucked.
	-	Programeer script is nu aangepast om ook het hardware ID van 9 bytes in het UICR te schrijven tijdens het programmeren van de firmware. 

2021-04-22 Release van versie 1.5.10
	-	Firmware release van de circulaire log implementatie. Bij voorgaande firmware versie werd de BEEPBASE log data op de MX flash geschreven tot deze vol was. Vervolgens moest de log worden gewist om meer data op te slaan. Dat is met deze update aangepast, zodat als de log de maximale grote bereikt, deze terug bij positie 0 de oude data begint te overschrijven. De logOffset waarde kan via BLE commando’s worden uitgelezen of worden geschreven. Als de log wordt uitgelezen kan de logOffset waarde worden gebruikt als extra controle om de overgang van nieuwe en oude data goed te laten verlopen bij het importeren van de data.

	-	Daarnaast zijn er extra BLE commando’s geïmplementeerd om het flash log vol te schrijven met simulatie waardes voor het testen van de functionaliteit. Ook is er een commando toegevoegd om de reset reason uit te lezen van de nRF52840 voor extra terug koppeling tijdens het testen.
		Nieuwe BLE commando’s:
	...
	-	BLE commando’s zijn gedocumenteerd in de BEEPBASE documentatie op Google Drive. Ook een beschrijving van de circulaire log en de fill log functionaliteit toegevoegd.


2021-04-09 Release van versie 1.5.9
	-	De RTC tijd wordt nu verstuurd in het opstart bericht

2021-04-09 Release van versie 1.5.8
	-	RTC timestamp probleem opgelost. Elke keer dat de tijd in seconde sinds de laatste RTC interrupt wordt berekend werd de afronding modulus van de 32kHz kristal opgeteld. Als 
		de functie voor het berekenen vaak genoeg werd aangeroepen lijdt dit tot een hoge tijd afwijking aangezien de modulus continue wordt opgeteld in plaats van enkel om de minuut.


2021-03-31 Release van versie 1.5.7
	-	Beep Control Point queue gebruikt nu het juiste BLE event om de berichten in de wachtrij te versturen nadat het vorige bericht is verstuurd.
	
	-	Dubbele berichten verholpen bij de volgende commando’s:
		1)	WRITE_TIME
		2)	WRITE_ON_STATE
		3)	WRITE_REED_STATE
...
2021-03-30 Release van versie 1.5.6
	-	Read flash commando 0x22 geeft nu status 0xE0F/3599d (End Of File) als response als het flash compleet is uitgelezen. Vorrheen was dit NRF_SUCCESS 0x0 Dit had ik gewijzigd
		tijdens het testen, omdat er een probleem was met herhalende response codes en er op verschillende plekken in de code de NRF_SUCCESS kon worden gegenereerd.
	
	-	BLE MTU size wordt nu door de BEEP karakteristiek gebruikt om de grote van een BLE bericht te limiteren, zodat het uitlezen van het flash niet standaard de maximale MTU size 
		gebruikt. Hierdoor zou het onder IOS nu ook goed moeten werken. Ik kon niet verder testen dan dat de MTU size goed stond, aangezien de nRF Connect app op IOS compleet andere
		logs genereerd.
		
	-	De Beep control point gebruikt nu een Queue. Hiermee worden berichten die naar de smartphone moeten worden gestuurd tijdelijk gebufferd. De queue kan maximaal
		4 berichten bufferen. Hiermee is het probleem dat er onder sommige situaties een vorige response code wordt herhaald verholpen.
		
	-	Flash clear commando 0x2100 werkt nu correct. Het flash wordt na het wissen opnieuw geinitialiseerd. Het flash reageerd hierna weer correct op commando's 0x22(get flash size) 
		en 0x20(read flash).


2021-03-15 Release van versie 1.5.5.
	-	Optimalizatie van de flash applicatie statemachine verlaagd waardoor het flash uitlezen ook correct werkt onder release builds. Bij eerdere firmware releases stopte de 
		statemachine doordat er een boolean werd gewist na een specifiek aantal berichten. Hiervoor geen duidelijke oorzaak gevonden dan dat met een lager optimalizatie niveau
		dan optimize for size het probleem niet meer optreed.
	
2021-03-12 Release van versie 1.5.4.
	-	Om de log uit te lezen moet een power reset niet meer nodig zijn.
	
	-	TX karakteristiek hoeft nu niet meer notificatie uit->aan te worden gezet als er wordt verbonden met een gebond apparaat. De notificatie staat wordt dan uit de 
		BLE stack gehaald.
	
	-	Als een flash commando niet wordt uitgevoerd, wordt er nu een error code terug gestuurd:
			NRF_ERROR_INVALID_STATE		Als de TX notificaties niet aan staan voor het uitlezen van het MX flash.
			NRF_ERROR_BUSY				Als de flash applicatie statemachine bezig is met een vorig commando.
			NRF_ERROR_INVALID_ADDR		Als er een invalide flash commando wordt gegeven.
		Dit moet ook voorkomen dat als het flash niet wordt uitgelezen en er geen staat of error code terug wordt gestuurd de BLE in een staat komt waarbij er geen nieuwe 
		berichten meer worden afgehandeld. Aan de smartphone kant resulteerde dit in de nRF connect App in GATT ERROR 128: No resources
		
	-	Ik kan met een kleine log file van 16879 bytes dit correct uitlezen en het aantal bytes komt overeen met de flash size die wordt uitgelezen met commando 0x22
