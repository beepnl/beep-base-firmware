...
NRF_QUEUE_DEF(beepQue_s, beep_queue, BEEP_QUEUE_LENGHT, NRF_QUEUE_MODE_NO_OVERFLOW);

// Queue function prototypes.
uint32_t ble_beep_controlPoint_addQueue(ble_beep_t * p_beep, BEEP_protocol_s * prot);
...
static uint16_t read_cccd(uint16_t conn_handle, uint16_t cccd_handle)
{
    uint32_t err_code;
    uint8_t  cccd_value_buf[BLE_CCCD_VALUE_LEN];
    bool     is_sccp_indic_enabled = false;
...
bool beep_read_txNotificationState(ble_beep_t * p_beep)
{
    uint16_t cccdState = read_cccd(p_beep->conn_handle, p_beep->tx_handles.cccd_handle);
    const bool indicationEnabled = ((cccdState & BLE_GATT_HVX_NOTIFICATION) != 0);
    NRF_LOG_INFO("Beep TX state %u/0x%04X, indication: %s", cccdState, cccdState, (indicationEnabled ? "On" : "Off"));
...
static uint32_t beep_ctrlpt_send(ble_beep_t * p_beep)
{
    ...
    while(!nrf_queue_is_empty(&beep_queue))
    {

        // Retrtieve data from the queue
        beepQue_s get;
        memset(&get, 0, sizeof(beepQue_s));
        err_code = nrf_queue_generic_pop(&beep_queue, &get, true);       // Just peek
        if(err_code != NRF_SUCCESS)
        {
            return err_code;
        }
        ...
        hvx_params.handle = p_beep->control_char.value_handle;
        hvx_params.type   = BLE_GATT_HVX_INDICATION;
        hvx_params.offset = 0;
        hvx_params.p_len  = &hvx_len;
        hvx_params.p_data = get.buff; //reponse;

        err_code = sd_ble_gatts_hvx(p_beep->conn_handle, &hvx_params);
        uint32_t queLenght = (uint32_t) nrf_queue_utilization_get(&beep_queue);
        #if 1
            NRF_LOG_INFO("Beep HVX: 0x%04X. queSize: %u", err_code, queLenght);
        #endif

        switch (err_code)
        {
            case NRF_SUCCESS:
                err_code = nrf_queue_generic_pop(&beep_queue, &get, false);       // Pop the previously peeked data.
                // Loop again if there' data in the Que
                break;
...
uint32_t beep_ctrlpt_send_error(ble_beep_t * p_beep, BEEP_CID cmd, uint32_t error)
{
    ...
    memset(&encode, 0, sizeof(BEEP_protocol_s));
    ...
    // Clear the buffer.
	ret = ble_beep_controlPoint_addQueue(p_beep, &encode);
	ret = beep_ctrlpt_send(p_beep);
    return ret;
}
...
static void on_connect(ble_beep_t * p_beep, ble_evt_t const * p_ble_evt)
{
    ...
    p_beep->isTX_notification_enabled = beep_read_txNotificationState(p_beep);
    p_beep->txEventHandler(APP_FLASH_COMM_CONNECTED, NULL);
    nrf_queue_reset(&beep_queue);
}
...
static void on_disconnect(ble_beep_t * p_beep, ble_evt_t const * p_ble_evt)
{
    ...
    p_beep->packetCounter                = 0;
    p_beep->mtuDatalenght = NRF_SDH_BLE_GATT_MAX_MTU_SIZE;
    p_beep->dataLenghtMax = BLE_NUS_MAX_DATA_LEN;
...
void ble_beep_on_ble_evt(ble_evt_t const * p_ble_evt, void * p_context)
{
    ...
    #if 0
        NRF_LOG_INFO("EEP BLE evnt=%u", (uint32_t) p_ble_evt->header.evt_id);
    #endif
...
/*@Brief: Function to encode and add the message to the response queue
*/
uint32_t ble_beep_controlPoint_addQueue(ble_beep_t * p_beep, BEEP_protocol_s * prot)
{
	uint32_t err_code;
    static uint32_t no_mem_count = 0;
    ...
    beepQue_s add;
    memset(&add, 0, sizeof(beepQue_s));

    err_code = beep_protocol_encode(true, prot, add.buff, &add.lenght, BEEP_MAX_LENGHT);
    if(err_code != NRF_SUCCESS)
    {
        return err_code;
    }
	...
	err_code = nrf_queue_push(&beep_queue, &add);
	if(err_code != NRF_SUCCESS)
	{
		NRF_LOG_INFO("Beep queue no mem: %u", no_mem_count++);
	}
	return err_code;
}
...
uint32_t ble_beep_controlPoint_send(ble_beep_t * p_beep, BEEP_protocol_s * prot)
{
    ...
    err_code = ble_beep_controlPoint_addQueue(p_beep, prot);
...
uint32_t ble_beep_TXdata_send(ble_beep_t * p_nus, uint8_t * p_data, uint16_t length)
{
    ret_code_t err_code;
...
uint32_t ble_beep_send_TXdata_fragment(ble_beep_t * p_nus)
{
    ...
    while(err_code == NRF_SUCCESS)
    {
        ...
        if(p_nus->txMsgLenght > p_nus->dataLenghtMax)
        {
            p_nus->txMsgLenght = p_nus->dataLenghtMax;
        }
