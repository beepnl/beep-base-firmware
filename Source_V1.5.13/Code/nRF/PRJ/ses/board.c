...
*/
void BoardDeInitMcu( const bool tiltswitch_wakeup, bool tilt_High )
{
    ret_code_t err_code;
        ...
        // Enable waking up from the tilt switch pin when the sensor is enabled and use the inverted polarity to wake-up from.
        if(tiltswitch_wakeup)
        {
            nrf_gpio_cfg_sense_input(SQ_SEN_645B_PIN,	NRF_GPIO_PIN_NOPULL, tilt_High ? NRF_GPIO_PIN_SENSE_LOW : NRF_GPIO_PIN_SENSE_HIGH);
        }
