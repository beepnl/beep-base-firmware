...
static uint32_t beep_protocol_log(BEEP_protocol_s * prot)
{
    ...
    switch(prot->command)
	{
        ...
        //-----------------------------------------------------------------------------
        case WRITE_TIME:
            NRF_LOG_INFO("Write Time: %s, %u/%0x%04X", get_logtime_string(prot->param.time), prot->param.time, prot->param.time);
            break;
...
uint32_t beep_protocol_decode(BEEP_protocol_s * prot, uint8_t * data, uint8_t msg_lenght, uint8_t * offset)
{
	...
	*offset += cmdLenght;
	...
	switch(prot->command)
	{
        ...
        {
            uint8_t on = data[1];
            prot->param.status.statusflag   = (~on) & 0x01;
            ret                             = NRF_SUCCESS;
            break;
        }
		...
		case WRITE_TIME:
        {
            prot->param.time                = uint32_big_decode(&data[1]);
            ret                             = NRF_SUCCESS;    
			break;
        }
...
uint32_t beep_protocol_encode(bool cmdPrepend, BEEP_protocol_s * prot, uint8_t * pBuffer, uint8_t * msg_lenght, uint8_t maxBufferLenght)
{
	...
	switch(prot->command)
	{
        ...
        case WRITE_SQ_MIN_STATE:
        {
            offset  += uint8_big_encode((~prot->param.status.statusflag) & 0x01, &data[offset]);
			break;
        }
