...
typedef uint32_t (* ble_tx_handler_t) (app_flash_evt_type_t eventType, BEEP_protocol_s * prot);
...
struct ble_beep_s
{
    ...
    volatile uint16_t               packetCounter;
};


/**@Brief: Read the TX notification state to check if notifications are enabled.
 */
